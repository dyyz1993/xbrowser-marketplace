...
```typescript
// 使用 Zustand selector
const rounds = useAgentStore(state => computeRoundsFromMessages(state.messages))

// 或者使用 Zustand selector 优化
import { createSelector } from 'zustand/selectors'

const selectRounds = createSelector([state => state.messages], messages =>
...
export const ChatArea: React.FC = () => {
  ...
  // 实时计算 rounds
  const rounds = useMemo(() => computeRoundsFromMessages(messages), [messages])
